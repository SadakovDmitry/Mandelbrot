...
int main()
{
    ...
    __m128 scale   = _mm_set1_ps (1200.f);
...
void Calculate_iters(float x_0, float y_0, union Iterations* iter, __m128 scale)
{
    __m128 arr_x_0 = _mm_div_ps( _mm_add_ps( _mm_set1_ps(x_0), _3210 ), scale );
    __m128 arr_y_0 = _mm_div_ps( _mm_set1_ps(y_0), scale );
    __m128 x_n     = arr_x_0;
    __m128 y_n     = arr_y_0;
    ...
    for (int n = 0 ; n < 256; n++)
    {
        ...
        __m128 res_cmp = _mm_cmp_ps(_mm_add_ps( x2, y2 ), RADIUS, _CMP_LT_OS );
        int mask       = _mm_movemask_ps(res_cmp);

        if(!mask) break;

        iter -> reg = _mm_add_epi32( iter -> reg, _mm_cvtps_epi32( _mm_and_ps( res_cmp, _mm_set1_ps( 1.f ))));
        __m128 temp = x_n;
...
void Check_keyboard(sf::RenderWindow &window , float* x_center, float* y_center, __m128* scale)
{
    ...
    while (window.pollEvent(event))
    {
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Equal))
        {
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::LShift ))
            {
                *scale     = _mm_mul_ps(*scale, _mm_set1_ps(2.2f));
                *x_center = (*x_center - WIDTH / 2) * 2.2 + WIDTH / 2;
                *y_center = (*y_center - HEIGHT / 2) * 2.2 + HEIGHT / 2;
            }
            else
            {
                *scale     = _mm_mul_ps(*scale, _mm_set1_ps(1.2f));
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Dash))
        {
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::LShift ))
            {
                *scale     = _mm_div_ps(*scale, _mm_set1_ps(2.2f));
                *x_center = (*x_center - WIDTH / 2) / 2.2 + WIDTH / 2;
                *y_center = (*y_center - HEIGHT / 2) / 2.2 + HEIGHT / 2;
            }
            else
            {
                *scale     = _mm_div_ps(*scale, _mm_set1_ps(1.2f));
                *x_center = (*x_center - WIDTH / 2) / 1.2 + WIDTH / 2;
                *y_center = (*y_center - HEIGHT / 2) / 1.2 + HEIGHT / 2;
            }
        }
